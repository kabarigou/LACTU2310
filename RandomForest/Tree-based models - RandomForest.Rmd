...
It can even be showed that for $ \lim_{B \rightarrow \infty} OOBE = \text{LLO-CV} $, that is the out-of-bag error converges to the LOO-CV error estimate (leave one out cross validation) which is unbiased but with higher variance with respect to k-fold CV with k<n. (see Hastie, T., Tibshirani, R., Friedman, J. (2001). The Elements of Statistical Learning.)
